"""Run GrabCut segmentation with Tree-Reweighted Belief Propagation (TRW-BP)."""
...
def _save_convergence_plot(curves: List[List[float]], output_path: Path) -> None:
    ...
    ax.set_title("TRW Convergence Curves (GrabCut)")
...
def run_experiment(args: argparse.Namespace) -> Dict[str, float]:
    ...
    save_metrics_csv(rows, results_dir / "grabcut_trw_metrics.csv")
    _save_convergence_plot(curves, plots_dir / "grabcut_trw_convergence_curves.png")
    ...
    with (results_dir / "grabcut_trw_summary.json").open("w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    print("\nGrabCut TRW experiment summary")

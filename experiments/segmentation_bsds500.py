...
def _save_triplet_plot(
    ...
    """Save qualitative plot for one sample."""
...
def _save_distribution_plot(values: List[float], title: str, xlabel: str, output_path: Path) -> None:
    ...
    ax.hist(values, bins=min(10, max(5, len(values))), color="#4C72B0", edgecolor="black", alpha=0.85)
    ax.set_title(title)
...
def _save_convergence_curves(curves: List[List[float]], output_path: Path) -> None:
    """Save per-sample and mean message-delta convergence curves."""
    if not curves:
        return
    ...
    for curve in curves:
        ...
        ax.plot(x, curve, color="#9AA7B1", alpha=0.45, linewidth=1.2)
    ...
    ax.set_title("BP Convergence Curves (Message Delta)")
...
def _save_selected_examples(samples: List[Dict[str, object]], output_path: Path) -> None:
    """Save best / median / worst IoU qualitative comparison panel."""
    if not samples:
        return
    ...
    fig.suptitle("BSDS500 Qualitative Comparison")
    plt.tight_layout()
    fig.savefig(output_path, dpi=170, bbox_inches="tight")
    plt.close(fig)
...
def run_experiment(args: argparse.Namespace) -> Dict[str, float]:
    ...
    _save_distribution_plot(iou_scores, "IoU Distribution (BSDS500)", "IoU", plots_dir / "iou_hist.png")
    _save_distribution_plot(
        acc_scores,
        "Pixel Accuracy Distribution (BSDS500)",
        "Accuracy",
